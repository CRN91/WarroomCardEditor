...
EFFECT_SCHEMA = {
    ...
    "grant_rails": [("amount", "int", "rail segments added to the player's stock")],
...
class FormBuilder:
    ...
    def _field(self, parent, r, key, kind, extra, value):
        if kind == "team":
            v2l = {v: lbl for lbl, v in TEAM_OPTIONS}
            l2v = {lbl: v for lbl, v in TEAM_OPTIONS}
            var = tk.StringVar(value=v2l.get(value, TEAM_OPTIONS[0][0]))
            ttk.Combobox(parent, textvariable=var, values=[lbl for lbl, _ in TEAM_OPTIONS],
                         state="readonly", width=16).grid(row=r, column=1, sticky="w", padx=4, pady=3)
            return lambda: l2v.get(var.get(), 1)
        if kind in ("text",):
...
class App(tk.Tk):
    ...
    def _tree_press(self, e):
        ...
        self._drag = {"card": iid[5:] if iid.startswith("card:") else None, "moved": False}

    def _tree_drag(self, e):
        if self._drag.get("card"):
            self._drag["moved"] = True
            self.tree.configure(cursor="exchange")

    def _tree_drop(self, e):
        self.tree.configure(cursor="")
        src = self._drag.get("card"); moved = self._drag.get("moved")
        self._drag = {"card": None, "moved": False}
        ...
        if target.startswith("set:"):
            ...
            tset, _ = self.deck.find_card(target[5:])
        else:
            return
        cur_set, _ = self.deck.find_card(src)
        if tset and tset != cur_set:
            self._commit_current()
            self.deck.move_card(src, tset)
            self._refresh_tree()
            try:
                self.tree.selection_set("card:" + src); self.tree.see("card:" + src)
            except Exception:
                pass
    ...
    def _build_body(self, card):
        ...
        if card.get("type") == "decision":
            ...
            for ck in keys:
                ...
                crow = ttk.Frame(lf); crow.pack(fill="x", pady=(2, 0))
                ttk.Label(crow, text="cost").pack(side="left")
                cv = tk.StringVar(value=str(ch.get("cost", "")) if ch.get("cost") else "")
                ttk.Entry(crow, textvariable=cv, width=8).pack(side="left", padx=4)
                self._choice_cost[ck] = cv
                ttk.Label(crow, text="pay from").pack(side="left", padx=(8, 0))
                pv = tk.StringVar(value=ch.get("pay_from", "pick_city"))
                ttk.Combobox(crow, textvariable=pv, values=PAY_PRESETS, state="readonly", width=10).pack(side="left", padx=4)
                self._choice_pay[ck] = pv
                ttk.Label(crow, text="(cost > 0 makes this a purchase — player picks a city to pay)",
                          foreground="#888").pack(side="left", padx=6)
    ...
    def _commit_current(self):
        ...
        w = self.v_weight.get().strip()
        if w and parse_num(w) != 1:
            c["weight"] = parse_num(w)
        else:
            c.pop("weight", None)
        ...
        for ck, var in self._choice_vars.items():
            if isinstance(c.get(ck), dict):
                c[ck]["label"] = var.get()
                cv = self._choice_cost.get(ck)
                cost_s = cv.get().strip() if cv else ""
                if cost_s and parse_num(cost_s) > 0:
                    c[ck]["cost"] = int(parse_num(cost_s))
                    pv = self._choice_pay.get(ck)
                    c[ck]["pay_from"] = (pv.get() if pv else "pick_city") or "pick_city"
                else:
                    c[ck].pop("cost", None)
                    c[ck].pop("pay_from", None)
        # id change
